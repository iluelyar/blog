...
function createCodeDivs(data) {
  ...
  Object.keys(data).forEach((lang) => {
    if (data[lang].show) {
      ...
      langBox.innerHTML = `<pre><code class="language-${lang}">${codeText
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")}</code></pre>`;
      codeBoxs.append(langBox);

      langDiv.addEventListener("click", function () {
        langBox.style.display =
          langBox.style.display === "none" ? "block" : "none";
      });
