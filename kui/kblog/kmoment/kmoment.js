export async function kmoment(el) {
  ...
  const [htmlText, cssText, dataList] = await Promise.all([
    fetch("/kui/kblog/kmoment/kmoment.html").then((res) => res.text()),
    fetch("/kui/kblog/kmoment/kmoment.css").then((res) => res.text()),
    fetch("/kui/kblog/kmoment/data.json").then((res) => res.json()),
  ]);
